...
class Msk:
    ...
    def __init__(self, last_time: str) -> None:
        self.__url = 'https://echo.msk.ru/news/'
        self.__domain = 'https://echo.msk.ru'
        self.source = 'Msk'
    ...
    def _get_news_feeds(self) -> list:
        def filter_fun(x):
            logger.debug(F"Msk filt: {x.time}>{self.last_time}  = {x.time.timestamp() > self.last_time.timestamp()}")
